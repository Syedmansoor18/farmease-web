    ...
    <div className="relative min-h-screen flex flex-col w-full overflow-hidden bg-[#F1F4F2]">
      ...
      <header className="relative z-10 w-full flex flex-col md:flex-row items-center justify-between px-5 md:px-16 pt-8 pb-6 gap-5">
        ...
        <div className="flex items-center justify-center gap-3 flex-wrap">
          ...
          <select
            ...
            className="bg-white/80 backdrop-blur-sm text-[#006F1D] font-bold py-2 px-3 md:px-4 rounded-xl shadow-sm outline-none cursor-pointer focus:ring-2 focus:ring-[#006F1D] border border-[#006F1D]/20 text-sm md:text-base"
          >
          ...
          <button
            ...
            className="text-[#2D3432] hover:text-[#006F1D] font-bold transition-colors text-sm md:text-base px-2"
          >
            {t('loginBtn')}
          </button>
          ...
          <button
            ...
            className="bg-[#006F1D] hover:bg-green-800 text-white font-bold py-2 px-5 md:px-6 rounded-full shadow-md transition-all active:scale-95 border border-[#00A82D]/20 text-sm md:text-base"
          >
            {t('signUpBtn')}
          </button>
        </div>
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-8 items-center flex-grow w-full px-5 md:px-16 pb-12 md:pb-16 text-center md:text-left mt-4 md:mt-0">
        ...
        <div className="space-y-6 flex flex-col items-center md:items-start">
          ...
          <p className="text-[#2D3432] text-base sm:text-lg md:text-xl max-w-lg leading-relaxed font-semibold">
            {t('heroDesc')}
          </p>

          <button
            onClick={() => navigate('/signup')}
            className="mt-4 bg-[#006F1D] hover:bg-green-800 transition-colors text-white font-extrabold py-4 px-10 rounded-full shadow-lg shadow-[#006F1D]/30 active:scale-95 tracking-wide w-full sm:w-auto"
          >
            {t('getStarted')}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-6 w-full max-w-lg mx-auto md:ml-auto mt-6 md:mt-0">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl md:rounded-3xl p-4 md:p-6 flex flex-col justify-center shadow-xl border border-[#DDE4E1]/50">
            ...
            <p className="text-[#59615F] font-bold text-xs md:text-sm">{t('activeFarmers')}</p>
          </div>

          <div className="bg-white/90 backdrop-blur-sm rounded-2xl md:rounded-3xl p-4 md:p-6 flex flex-col justify-center shadow-xl border border-[#DDE4E1]/50">
            <h3 className="text-3xl md:text-4xl font-black text-[#006F1D] mb-1 md:mb-2">5K+</h3>
            <p className="text-[#59615F] font-bold text-xs md:text-sm">{t('equipmentListings')}</p>
          </div>

          <div className="bg-white/90 backdrop-blur-sm rounded-2xl md:rounded-3xl p-4 md:p-6 flex flex-col justify-center shadow-xl border border-[#DDE4E1]/50">
            <h3 className="text-3xl md:text-4xl font-black text-[#006F1D] mb-1 md:mb-2">1K+</h3>
            <p className="text-[#59615F] font-bold text-xs md:text-sm">{t('dailyTransactions')}</p>
          </div>

          <div className="rounded-2xl md:rounded-3xl p-4 md:p-6 flex items-end min-h-[140px] md:min-h-[160px] overflow-hidden relative shadow-xl border border-[#DDE4E1]/50">
             <div
               className="absolute inset-0 z-0"
               style={{
             ...
             <span className="text-white font-bold z-20 relative text-sm md:text-lg tracking-wide">{t('smartFarming')}</span>
          </div>
        </div>
      </div>
    </div>
