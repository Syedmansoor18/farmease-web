...
const EquipmentPostingPage = () => {
  ...
  const handleUseCurrentLocation = () => {
    if (!navigator.geolocation) { showToast(t('geoNotSupported') || "Geolocation not supported."); return; }
    showToast(t('detectingLocation') || "Detecting location...");
    navigator.geolocation.getCurrentPosition(
      () => showToast(t('locationDetected') || "Location detected!"),
      () => showToast(t('locationFailed') || "Could not detect. Please enter manually.")
    );
  };
 ...
 const readFile = (file) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (farmerError || !farmerData) throw new Error(t('farmerProfileNotFound') || "Could not find your Farmer profile.");
    ...
    <div className="bg-gray-50 min-h-screen flex" style={{ maxWidth: "100vw", overflowX: "hidden" }}>
      ...
      <div className="flex-1 min-w-0 py-5 px-5 ml-20">
        ...
        <div className="bg-white border border-gray-100 rounded-xl px-4 py-3 mb-4">
          <p className="text-xs text-gray-500 mb-2">{t('contactInfo') || "Contact Information"}</p>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700 flex-1">
              +91 98765 43210{" "}
              <span className="text-xs text-gray-400 ml-1">{t('primaryNumber') || "Primary number"}</span>
            </span>
          ...
          <button className="mt-3 w-full text-sm text-green-700 font-medium flex items-center justify-center gap-1 hover:underline">
            ...
            {t('addAlternateNumber') || "Add Alternate Number"}
          </button>
        </div>
